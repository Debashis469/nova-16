module CPU (
    ...
    ARegister inst6 (.in(aRegIn), .load(loadA), .out(aRegOut));
    assign addressM = aRegOut[14:0]; // out[0..14]=addressM

    // D register
    And inst7 (.a(isCInstr), .b(instruction[4]), .out(loadD));
    DRegister inst8 (.in(aluOut), .load(loadD), .out(dRegOut));
    ...
    // Program counter
    wire [15:0] pcFull;
    PC inst20 (.in(aRegOut), .load(doJump), .inc(1'b1), .reset(reset), .out(pcFull));
